    ...
    try {
      ...
      if (contentType === 'image') {
        ...
        if (payload.videoFrames && payload.videoFrames.length > 0) {
          for (const frame of payload.videoFrames) {
        ...
        const frameCount = userContent.length; // number of image items pushed above
        userContent.push({
          type: 'input_text',
          text: frameCount > 1
            ? `Analyse these ${frameCount} video frames (sampled at 0.25s intervals). Are they from an AI-generated video? Respond with JSON only: {"score": <0.0-1.0>, "label": "<ai|human|uncertain>"}`
            : 'Is this video frame from an AI-generated video? Respond with JSON only: {"score": <0.0-1.0>, "label": "<ai|human|uncertain>"}',
        });
      ...
      if (!response.ok) {
        ...
        console.warn(`[RealityCheck] Azure OpenAI API HTTP ${response.status}:`, errorBody);
      ...
      console.warn('[RealityCheck] Azure OpenAI classify error:', msg);
      return { score: 0.5, label: 'error' };
    }
  }
}
