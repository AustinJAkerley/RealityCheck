...
/**
 * Classify a video frame using Azure OpenAI Responses API (vision).
 *
 * Accepts a representative frame from the video (as a data-URL or source URL)
 * and asks the vision model to determine whether the video is AI-generated.
 *
 * @param config        Azure OpenAI configuration.
 * @param imageDataUrl  Base64 data-URL of the captured video frame.
...
export async function classifyVideoWithAzureOpenAI(
  ...
  const systemPrompt =
    'You are an AI-generated video detector. Analyse the provided video frame and determine whether it was ' +
