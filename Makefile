...
.PHONY: help install build build-core build-api build-chrome build-edge build-firefox build-safari test test-api clean
...
help:
	...
	@echo "  make build            Build core library + API service + all four extensions"
	@echo "  make build-core       Build shared core library only"
	@echo "  make build-api        Build Azure API service → packages/api/dist/"
	...
	@echo "  make test             Run all unit tests (core + API)"
	@echo "  make test-api         Run API service unit tests only"
